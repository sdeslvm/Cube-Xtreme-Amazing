...
protocol WebLoadable: AnyObject {
    var state: CubeXtremeWebStatus { get set }
...
final class CubeXtremeWebLoader: NSObject, ObservableObject, WebLoadable, ProgressMonitoring {
    // MARK: - Свойства

  @Published var state: CubeXtremeWebStatus = .standby
    ...
    func observeProgression() {
        progressPublisher
            ...
            .sink { [weak self] progress in
                ...
                let newState: CubeXtremeWebStatus = progress < 1.0 ? .progressing(progress: progress) : .finished
...
extension CubeXtremeWebLoader: WKNavigationDelegate {
...
extension CubeXtremeWebLoader {
    ...
    func publish(_ newState: CubeXtremeWebStatus) {
...
private extension CubeXtremeWebLoader {
    func updateState(_ newState: CubeXtremeWebStatus) {
