...
struct CubeXtremeWebViewBox: UIViewControllerRepresentable {
    // MARK: - Свойства

    @ObservedObject var loader: CubeXtremeWebLoader
    var defaultOrientations: UIInterfaceOrientationMask = .landscape

    // MARK: - Координатор

    func makeCoordinator() -> CubeXtremeWebCoordinator {
        CubeXtremeWebCoordinator { [weak loader] status in
    ...
    func makeUIViewController(context: Context) -> CubeXtremeBaseWebViewController {
        let configuration = createWebViewConfiguration()
        return CubeXtremeBaseWebViewController(
    ...
    func updateUIViewController(_ uiViewController: CubeXtremeBaseWebViewController, context: Context) {
