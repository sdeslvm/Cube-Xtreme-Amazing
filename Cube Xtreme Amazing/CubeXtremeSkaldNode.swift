...
protocol CubeXtremeLoadStateRepresentable {
    var type: CubeXtremeLoadState.StateType { get }
...
struct CubeXtremeLoadState: Equatable, CubeXtremeLoadStateRepresentable {
    ...
    static func idle() -> CubeXtremeLoadState {
        CubeXtremeLoadState(type: .idle, percent: nil, error: nil)
    }

    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> CubeXtremeLoadState {
        CubeXtremeLoadState(type: .progress, percent: percent, error: nil)
    }

    /// Создание состояния успеха
    static func success() -> CubeXtremeLoadState {
        CubeXtremeLoadState(type: .success, percent: nil, error: nil)
    }

    /// Создание состояния ошибки
    static func error(_ err: String) -> CubeXtremeLoadState {
        CubeXtremeLoadState(type: .error, percent: nil, error: err)
    }

    /// Создание состояния отсутствия подключения
    static func offline() -> CubeXtremeLoadState {
        CubeXtremeLoadState(type: .offline, percent: nil, error: nil)
    }

    // MARK: - Методы сравнения

    /// Пользовательская реализация сравнения
    func isEqual(to other: CubeXtremeLoadState) -> Bool {
    ...
    static func == (lhs: CubeXtremeLoadState, rhs: CubeXtremeLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension CubeXtremeLoadState: CustomStringConvertible {
